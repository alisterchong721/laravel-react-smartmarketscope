...
from smartmarketscope_quant.nested_research.cycle2.candidates import (
    expand_cycle2_candidates,
)
from smartmarketscope_quant.nested_research.cycle2.data import (
    load_cycle2_barrier_samples,
    load_cycle2_daily_samples,
)
...
def _load_samples(repo_root: Path, config: dict, experiment: dict):
    ...
    if family == "H1_BARRIER_ASYMMETRY_TREE":
        return load_cycle2_barrier_samples(repo_root, config)[0]
    include_h1 = family in {
        "H1_ALIGNED_DAILY_MOMENTUM",
        "RANGE_EDGE_DAILY_MOMENTUM",
        "MOMENTUM_ADVERSE_RISK_VETO_LOGISTIC",
    }
    return load_cycle2_daily_samples(
        repo_root, config, include_h1_features=include_h1
    )
...
def _expected_reasons(experiment: dict, result: dict, config: dict) -> set[str]:
    ...
    if experiment["family"] == "MOMENTUM_ADVERSE_RISK_VETO_LOGISTIC":
    ...
    if experiment["family"] == "H1_BARRIER_ASYMMETRY_TREE":
        probability = result["outer_probability_metrics"]
        if probability is None:
            reasons.add("OUTER_MULTICLASS_PROBABILITY_METRICS_UNAVAILABLE")
        else:
            if (
                probability["multiclass_brier_score"]
                >= probability["reference_multiclass_brier_score"]
            ):
                reasons.add("OUTER_MULTICLASS_BRIER_NO_SKILL")
            if probability["log_loss"] >= probability["reference_log_loss"]:
                reasons.add("OUTER_MULTICLASS_LOG_LOSS_NO_SKILL")
        candidate_q = float(np.quantile(fold_nets, 0.25))
        control_ids = sorted(
            {key for fold in folds for key in fold.get("controls", {})}
        )
        for control_id in control_ids:
            control_nets = [
                fold.get("controls", {})
                .get(
                    ...
                    {
                        "metrics": {
                            "net_by_scenario": {"NORMALIZED_MEDIUM_COST": 0}
                        }
                    },
                )["metrics"]["net_by_scenario"]["NORMALIZED_MEDIUM_COST"]
                for fold in folds
            ]
            if candidate_q <= float(np.quantile(control_nets, 0.25)):
                reasons.add(
                    f"NON_ML_CONTROL_LOWER_QUARTILE_NOT_BEATEN:{control_id}"
                )
    return reasons
...
def _validate_registry_lifecycle(
    ...
    _require(states in [running, completed], "Unexpected cycle-two registry lifecycle")
    if states == running:
        return
    ...
    _require(
        terminal_payload.get("robustness_metrics")
        == "NOT_APPLICABLE_NO_PHASE_J_CYCLE2_SURVIVOR",
        "Phase J terminal robustness handoff mismatch",
    )
    _require(
        terminal_payload.get("prop_metrics")
        == "NOT_APPLICABLE_NO_PHASE_J_CYCLE2_SURVIVOR",
        "Phase J terminal prop handoff mismatch",
    )


def validate_phase_j_cycle2(repo_root: Path, config_path: Path) -> dict:
    repo_root = repo_root.resolve()
    config_path = config_path.resolve()
    config = json.loads(config_path.read_text(encoding="ascii"))
    summary_path = repo_root / "research/artifacts/nested/phase_j_cycle2/summary.json"
    ...
    _require(
        summary["experiment_count"] == 6
        and summary["candidate_trial_count"] == 22,
        "Cycle-two Phase J budget mismatch",
    )
    ...
    for experiment_id, result in summary["experiments"].items():
        ...
        _require(
            result["candidate_trial_count"]
            == len(expand_cycle2_candidates(experiment)),
            f"Trial count mismatch for {experiment_id}",
        )
        ...
        for fold_config, fold in zip(config["outer_walk_forward"]["folds"], result["outer_folds"]):
            ...
            manifest_path = (
                repo_root
                / "research/artifacts/nested/phase_j_cycle2/split_manifests"
                / experiment_id
                / f"{fold['fold_id']}.json.gz"
            )
            ...
            for row in rows:
                ...
                if row["class_probabilities"]:
                    values = json.loads(row["class_probabilities"])
                    _require(
                        len(values) == 3
                        and all(0 <= float(value) <= 1 for value in values),
                        "Inner three-way probability is invalid",
                    )
                    _assert_close(sum(float(value) for value in values), 1.0, "Inner class probabilities do not sum to one")
                    if int(row["predicted_action"]) == 0:
                        _require(not accepted, "Predicted timeout did not abstain")
        ...
        for row in outer_rows:
            ...
            elif row["class_probabilities"]:
                values = tuple(float(value) for value in json.loads(row["class_probabilities"]))
                _require(len(values) == 3, "Outer three-way probability shape mismatch")
                _assert_close(sum(values), 1.0, "Outer class probabilities do not sum to one")
                class_probability_values.append(values)
                probability_targets.append(int(row["target"]))
                if int(row["predicted_action"]) == 0:
                    _require(not accepted, "Outer predicted timeout did not abstain")
        outer_prediction_rows += len(outer_rows)
        if experiment["family"] == "MOMENTUM_ADVERSE_RISK_VETO_LOGISTIC":
            if outer_rows:
                _require(len(probability_values) == len(outer_rows), "Risk-veto outer probabilities are incomplete")
                _require(result["outer_probability_metrics"] is not None, "Risk-veto probability metrics are missing")
                ...
                _require(not probability_values, "Risk-veto probabilities exist without outer rows")
                _require(result["outer_probability_metrics"] is None, "Risk-veto empty outer metrics are not null")
        if experiment["family"] == "H1_BARRIER_ASYMMETRY_TREE":
            if outer_rows:
                _require(
                    len(class_probability_values) == len(outer_rows),
                    "Barrier-tree outer class probabilities are incomplete",
                )
                _require(result["outer_probability_metrics"] is not None, "Barrier-tree probability metrics are missing")
                values = np.asarray(class_probability_values, dtype=float)
                class_index = {-1: 0, 0: 1, 1: 2}
                indices = np.asarray(
                    [class_index[target] for target in probability_targets], dtype=int
                )
                actual = np.zeros_like(values)
                actual[np.arange(len(indices)), indices] = 1.0
                brier = float(np.mean(np.sum((values - actual) ** 2, axis=1)))
                log_loss = float(
                    -np.mean(np.log(np.clip(values[np.arange(len(indices)), indices], 1e-12, 1.0)))
                )
                _assert_close(
                    brier,
                    result["outer_probability_metrics"]["multiclass_brier_score"],
                    "Outer multiclass Brier mismatch",
                )
                _assert_close(
                    log_loss,
                    result["outer_probability_metrics"]["log_loss"],
                    "Outer multiclass log-loss mismatch",
                )
            else:
                _require(not class_probability_values, "Barrier-tree probabilities exist without outer rows")
                _require(result["outer_probability_metrics"] is None, "Barrier-tree empty outer metrics are not null")
    ...
    _require(
        _canonical_hash(core) == summary["core_results_sha256"],
        "Cycle-two Phase J core result hash mismatch",
    )
    _require(
        (repo_root / "CYCLE_2_GOVERNANCE_DEVIATION.md").is_file(),
        "Cycle-two orchestration deviation disclosure missing",
    )
    result = {
        "schema_version": "1.0.0",
        "artifact_id": "PHASE-J-CYCLE-2-INDEPENDENT-VALIDATION-QRP-20260712",
        ...
        "governance_caveat": "CYCLE_2_GOVERNANCE_DEVIATION.md",
        "final_holdout_access_count": 0,
        "paper_trading_started": False,
        "live_execution_started": False,
    }
    output = repo_root / "research/artifacts/nested/phase_j_cycle2/validation.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Independently validate cycle-two Phase J nested artifacts"
    )
    parser.add_argument("--repo-root", type=Path, default=Path.cwd())
    parser.add_argument(
        "--config", type=Path, default=Path("research/config/phase_j_cycle2.json")
    )
    args = parser.parse_args()
    root = args.repo_root.resolve()
    config = args.config if args.config.is_absolute() else root / args.config
    print(json.dumps(validate_phase_j_cycle2(root, config), indent=2, sort_keys=True))
